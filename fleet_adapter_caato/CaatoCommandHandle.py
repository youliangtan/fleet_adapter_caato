...
# States for CaatoCommandHandle's state machine used when guiding robot along
# a new path
class CaatoState(enum.IntEnum):
    IDLE = 0
    WAITING = 1
    MOVING = 2
...
##############################################################################
class CaatoCommandHandle(adpt.RobotCommandHandle):
    def __init__(self,
                 name,
                 config,
                 node,
        ...
        # The Caato robot has various cleaning modes. Here we turn off the
    ...
    def follow_new_path(
        ...
        # positions but different orientations. The caato robot is unable to
        ...
        def _follow_path():
            ...
            while (self.remaining_waypoints or
                   self.state == CaatoState.MOVING or
                   self.state == CaatoState.WAITING):
                ...
                elif self.state == CaatoState.WAITING:
                    time.sleep(0.1)
                ...
                elif self.state == CaatoState.MOVING:
                    time.sleep(0.5)
    ...
    def dock(
        ...
        def _dock():
            ...
            while True:
                ...
                # The Caato75 requires a TF path to dock to charging location. This path is
                # named the same as the dock name (charger_caato75_x)
                if self.name[:8] == "caato75":
                    self.api.set_cleaning_mode(self.config['inactive_cleaning_config'])
                    if self.api.start_task(dock_name, self.robot_map_name):
                        break
                # For Caato40 and 50, we will use the navigate_to_waypoint API to
