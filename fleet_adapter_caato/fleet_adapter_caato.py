...
from .CaatoCommandHandle import CaatoCommandHandle
from .CaatoClientAPI import CaatoAPI
...
def initialize_fleet(config_yaml, nav_graph_path, node, server_uri, args):
    ...
    vehicle_traits = traits.VehicleTraits(
        ...
        profile=profile)
    # Caato robots are unable to reverse
    ...
    # Adapter
    adapter = adpt.Adapter.make('caato_fleet_adapter')
    if args.use_sim_time:
        adapter.node.use_sim_time()

    assert adapter, ("Unable to initialize caato adapter. Please ensure "
    ...
    def _add_fleet_robots():
        ...
        while len(missing_robots) > 0:
            ...
            for robot_name in list(missing_robots.keys()):
                # sam: iterate thru list of robots in config file
                node.get_logger().debug(f"Connecting to robot: {robot_name}")
                robot_config = missing_robots[robot_name]['caato_config']
                ...
                caato_pos = api.position()
                if caato_pos is None:
                ...
                for map_name, tf in robot_map_tf.items():
                    ...
                    if "reference_coordinates" in tf:
                        ...
                        caato_coords = tf['reference_coordinates']['robot']
                        mse = rmf_transform.estimate(caato_coords, rmf_coords)
                    ...
                    print(f"RMF to Caato transform :: trans [{tx}, {ty}]; rot {r}; scale {s}")
                    tx, ty, r, s = rmf_transform.to_rmf_map_transform()
                    print(f"Caato to RMF transform :: trans [{tx}, {ty}]; rot {r}; scale {s}")
...
def main(argv=sys.argv):
    ...
    parser = argparse.ArgumentParser(
        prog="fleet_adapter_caato",
        description="Configure and spin up fleet adapter for Gaussian Caato robots ")
    ...
    parser.add_argument("-tf", "--test_api_config_file", type=str, required=False, default="",
        help='supply a test_client_api config file to test caato client api as sim')
    ...
    node = rclpy.node.Node('caato_command_handle')
